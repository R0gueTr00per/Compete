...
<div
    x-id="['input']"
    x-init="$nextTick(() => { let i = $el.querySelector('input'); if (i) { i.setAttribute('inputmode', 'search'); i.setAttribute('enterkeyhint', 'search'); } })"
        ...
        <x-filament::input
            :attributes="
                (new ComponentAttributeBag)->merge([
                    ...
                    'x-bind:id' => '$id(\'input\')',
                    'x-on:keyup' => 'if ($event.key === \'Enter\') { $wire.$refresh(); $el.blur(); }',
                ])
            "
        />
    </x-filament::input.wrapper>
