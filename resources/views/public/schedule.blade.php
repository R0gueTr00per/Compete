<x-layouts.public>
    ...
    @php
        $compDateStr  = tenant_date($competition->competition_date);
    ...
    <div x-data="{ selected: null, day: '{{ $initialDay }}', dayInfo: {{ json_encode($dayInfoJs) }} }" @sched-panel-close.window="selected = null">
    ...
    @else
        @php
            ...
            $buildDayTimeline = function ($dayDivisions, $dayDate, $dayId = null) use ($breaks) {
                $dayBreaks = $breaks
                ...
                $timeline = [];
                $bIdx = 0;
                foreach (collect($dayDivisions)->sortBy('running_order') as $div) {
            ...
            if ($compDays->isNotEmpty()) {
                ...
                foreach ($divisions as $location => $locationDivisions) {
                    foreach ($compDays as $cday) {
                        $dayDate = \Carbon\Carbon::parse($cday->date)->format('Y-m-d');
                        $perLocPerDay[$location][$cday->id] = $buildDayTimeline(
                            collect($locationDivisions)->where('competition_day_id', $cday->id),
                            $dayDate,
                            $cday->id
                        );
                    }
            ...
            // Fallback single-day breaks (only used when $compDays is empty)
        ...
        @endphp
            ...
            {{-- Day filter --}}
            @if ($compDays->isNotEmpty())
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-2 flex-wrap border-b border-gray-200">
                    @foreach ($compDays as $cday)
                        <button type="button"
            ...
            <div class="sm:hidden px-3 pt-4" :class="selected !== null ? 'pb-56' : 'pb-6'">
                <div class="flex gap-1.5">
                    @foreach ($divisions as $location => $locationDivisions)
                        @php
                            if ($compDays->isEmpty()) {
                                $mobileTimeline = $buildDayTimeline(collect($locationDivisions)->sortBy('running_order'), $compDate);
                            }
                        @endphp
                                ...
                                @if ($compDays->isNotEmpty())
                                    @foreach ($compDays as $cday)
                                        <div x-show="day === '{{ $cday->id }}'">
                                            @foreach ($perLocPerDay[$location][$cday->id] as $row)
                                                @if ($row['type'] === 'break')
                                                    <div class="w-full rounded bg-amber-50 border border-amber-200 px-1 py-1.5 text-center mb-1">
            ...
            <div class="hidden sm:block max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div class="flex gap-4 overflow-x-auto pb-4 items-start">
                    @foreach ($divisions as $location => $locationDivisions)
                        @php
                            if ($compDays->isEmpty()) {
                                $desktopTimeline = $buildDayTimeline(collect($locationDivisions)->sortBy('running_order'), $compDate);
                            }
                        @endphp
