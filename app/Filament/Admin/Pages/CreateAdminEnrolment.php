...
class CreateAdminEnrolment extends Page implements HasForms
{
        ...
        return $form->schema([
            ...
            Section::make('Competition entry details')
                ...
                ->schema([
                    ...
                    Select::make('rank_id')
                        ->label('Rank / Level')
                        ->options(fn () => Rank::where('organisation_id', Competition::find($this->competition_id)?->organisation_id)->orderBy('sort_order')->pluck('name', 'id'))
                        ->required()
                        ->searchable()
                        ->live()
                        ->afterStateUpdated(function () {
    ...
    public function submit(): void
    {
        ...
        $this->reset(['selected_entries', 'details_confirmed', 'competitor_profile_id',
            ...
            'dojo_type', 'dojo_name', 'guest_style', 'rank_id', 'weight_kg']);
    }
}
