<?php

namespace App\Filament\Admin\Resources;

use App\Filament\Admin\Resources\CompetitionEventResource\Pages;
use App\Filament\Admin\Resources\CompetitionEventResource\RelationManagers;
use App\Models\CompetitionEvent;
use Filament\Forms\Components\Select;
use Filament\Forms\Components\TextInput;
use Filament\Forms\Form;
use Filament\Resources\Resource;
use Filament\Tables\Columns\TextColumn;
use Filament\Tables\Table;

class CompetitionEventResource extends Resource
{
    protected static ?string $model = CompetitionEvent::class;
    protected static ?string $navigationIcon = 'heroicon-o-rectangle-stack';
    protected static ?string $navigationGroup = 'Competitions';
    protected static bool $shouldRegisterNavigation = false; // Accessed via Competition only

    public static function form(Form $form): Form
    {
        return $form->schema([
            TextInput::make('name')->label('Event type name')->disabled(),

            TextInput::make('running_order')->numeric()->nullable(),
            TextInput::make('location_label')->maxLength(50)->nullable(),
            TextInput::make('target_score')->numeric()->nullable(),

            Select::make('status')
                ->options([
                    'scheduled' => 'Scheduled',
                    'running'   => 'Running',
                    'complete'  => 'Complete',
                    'cancelled' => 'Cancelled',
                ])
                ->required(),
        ]);
    }

    public static function table(Table $table): Table
    {
        return $table->columns([
            TextColumn::make('name'),
            TextColumn::make('status')->badge(),
        ]);
    }

    public static function getRelationManagers(): array
    {
        return [
            RelationManagers\DivisionsRelationManager::class,
        ];
    }

    public static function getPages(): array
    {
        return [
            'index' => Pages\ListCompetitionEvents::route('/'),
            'edit'  => Pages\EditCompetitionEvent::route('/{record}/edit'),
        ];
    }
}
