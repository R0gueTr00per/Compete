...
class LfpRound1Seeder extends Seeder
{
    public function run(): void
    {
        ...
        $events = [];
        foreach ([
            ['name' => 'Kata',               'order' => 1, 'location' => 'Mat 1'],
            ['name' => 'Tile Breaking',      'order' => 2, 'location' => 'Mat 1'],
            ['name' => 'Yakusuko',           'order' => 3, 'location' => 'Mat 2'],
            ['name' => 'Semi Contact',       'order' => 4, 'location' => 'Mat 2'],
            ['name' => 'Point Sparring',     'order' => 5, 'location' => 'Mat 3'],
            ['name' => 'Continuous Sparring','order' => 6, 'location' => 'Mat 3'],
            ['name' => 'Sumo',              'order' => 7, 'location' => 'Mat 4'],
        ] as $cfg) {
            $et = EventType::where('name', $cfg['name'])->first();
            if (! $et) {
                continue;
            }
            $events[$cfg['name']] = CompetitionEvent::updateOrCreate(
                ['competition_id' => $competition->id, 'event_type_id' => $et->id],
                ['running_order' => $cfg['order'], 'location_label' => $cfg['location'], 'status' => 'scheduled']
            );
        }
